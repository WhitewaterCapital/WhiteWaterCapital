...
// maplibre-gl v6 dropped its default export — import the namespace (all named
// exports: Map, Marker, NavigationControl, …).
...
const MAP_STYLE = {
  version: 8,
  sources: {
    dark: { type: 'raster', tiles: ['https://cartodb-basemaps-a.global.ssl.fastly.net/dark_nolabels/{z}/{x}/{y}.png'], tileSize: 256 },
    light: { type: 'raster', tiles: ['https://cartodb-basemaps-a.global.ssl.fastly.net/light_nolabels/{z}/{x}/{y}.png'], tileSize: 256 },
...
export default function WarMapClient() {
  ...
  useEffect(() => {
    ...
    // Reveal the map as soon as the style is up; apply the globe projection
    // defensively (maplibre v6 can throw on the projection call — a flat map is
    // still fully usable, so never let it block the reveal).
    const applyGlobe = () => {
      try {
        if (typeof map.setProjection === 'function') map.setProjection({ type: 'globe' });
      } catch {
        /* globe optional — flat map still works */
      }
    };
    map.on('style.load', () => { applyGlobe(); setMapLoaded(true); });
    map.on('load', () => { applyGlobe(); setMapLoaded(true); });
    return () => {
      map.remove();
      mapRef.current = null;
    };
    ...
    <div className="ww-root">
      ...
      <header className="ww-topbar">
        <div className="ww-brand">
          <a href="/dashboard" className="ww-back" title="Back to the Desk">← Desk</a>
...
// Lifted to module scope: it was defined inside WarMapClient, so the War Map's
// per-second LIVE clock re-render remounted it every second — resetting its
// state to [] and refetching (the cause of the endless "No live items yet" and
// the fetch storm).
function MiniFeed() {
...
.ww-back { color: var(--ww-text-dim); font-family: ui-monospace, monospace; font-size: 11px; text-decoration: none; margin-right: 12px; padding: 5px 9px; border: 1px solid var(--ww-border); border-radius: 5px; transition: color .15s, border-color .15s; }
.ww-back:hover { color: var(--ww-text); border-color: var(--ww-text-dim); }
